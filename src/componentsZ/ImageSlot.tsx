...
export const ImageSlot: React.FC<ImageSlotProps> = ({ uri, index, onPress }) => {
  return (
    <View className="w-full h-52 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden items-center justify-center relative mb-4">
      {uri ? (
        <>
          <Image
            source={{ uri }}
        ...
        <TouchableOpacity
          activeOpacity={0.7}
          onPress={() => onPress(index)}
          className="w-full h-full items-center justify-center bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl"
        >
